...
\begin{enumerate}
\item No generation (ex. \textit{Super Mario Bros} (Nintendo, 1985) where everything was handplaced, drawn and animated as explained in \textit{Super Mario Bros}. Level 1-1\cite{EurogamerMiyamotoInterview})
\item Content generation in the game making phase. (ex. \textit{The Elder Scrolls Oblivion} (Bethesda Softworks, 2006) used \gls{pcg} to generated most of the world before the artists curated it.\cite{PCGamerCarterInterview} An example of a widely used \gls{pcg} algorithm middleware for game studios is SpeedTree\cite{SpeedTree}
\item Gameplay partially based on \gls{pcg}
\item Games almost completely generated ex. \textit{Dwarf Fortress} (Dwarf Fortress, 2006)
\end{enumerate}
